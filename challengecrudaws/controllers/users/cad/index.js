...
module.exports = {
    ...
    async insertContacts(req,res){
        const usersId = req.params.id;
        const { phone, cell, whatsapp } = req.body;
        
        const users = await Users.findByPk(usersId);
        if(!users)
            res.status(400).json({ message: 'Usuário não encontrado' });
        
        const contactsMain = await ContactsMain.create({
            phone,
            cell,
            whatsapp,
            usersId
        ...
        return res.json(contactsMain);
    },
    async insertSocials(req,res){
        const usersId = req.params.id;
        const { facebook, instagram, linkedin, pinterest } = req.body;
        
        const users = await Users.findByPk(usersId);
        if(!users)
            res.status(400).json({ message: 'Usuário não encontrado' });
        
        const contactsSocial = await ContactsSocial.create({
            facebook,
            instagram,
            linkedin,
            pinterest,
        ...
        return res.json(contactsSocial);
    },
    ...
    async viewAllWithPosition (req,res) {
        try {
    ...
    async viewAllWithContacts (req,res) {
        try {
            const id = req.params.id;
            const users = await Users.findByPk(id,{
                include: [ 'contacts' ],
    ...
    async viewAllWithSocials (req,res) {
        try {
            const id = req.params.id;
            const users = await Users.findByPk(id,{
                include: [ 'socials' ],
